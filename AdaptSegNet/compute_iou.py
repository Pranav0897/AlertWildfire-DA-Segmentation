...
def iou_pytorch(outputs: torch.Tensor, labels: torch.Tensor):
    ...
    SMOOTH = 1e-6
    intersection = torch.count_nonzero(torch.bitwise_and(outputs>0, labels>0)) #.double().sum()  # Will be zero if Truth=0 or Prediction=0
    union = torch.count_nonzero(torch.bitwise_or(outputs>0, labels>0)) #.double().sum()         # Will be zzero if both are 0
